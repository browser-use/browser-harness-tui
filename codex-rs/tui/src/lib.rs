...
/// Workspaces generated by `browser-harness agent`/`tui` are trusted by
/// construction, so they never prompt.
fn should_show_trust_screen(config: &Config) -> bool {
    if let Ok(agent_root) = std::env::var("BROWSER_HARNESS_AGENT_ROOT")
        && !agent_root.trim().is_empty()
        && config.cwd.as_path() == std::path::Path::new(&agent_root)
    {
        return false;
    }
