...
// The Chat Completions endpoint reuses the same request-shaping options as the
// Responses endpoint (session/thread ids, extra headers, compression). The
// `turn_state` field is ignored because Chat has no sticky-routing turn token.
pub use crate::endpoint::responses::ResponsesOptions as ChatCompletionsOptions;
