...
impl HickoryDns {
    ...
    pub fn try_new_system() -> Result<Self, OpaqueError> {
        ...
        Ok(TokioResolver::builder_tokio()
            ...
            .build()
            .context("build async dns resolver from system conf builder")?
            .into())
    }
}
...
impl HickoryDnsBuilder {
    ...
    pub fn build(self) -> HickoryDns {
        ...
        // `ResolverBuilder::build` is only fallible when hickory-resolver is built
        // with one of its TLS features (`__tls`), which this crate never enables:
        // see the `hickory-resolver` dependency in Cargo.toml, which pins
        // `default-features = false` with only `tokio` and `system-config`.
        HickoryDns(Arc::new(resolver_builder.build().expect(
            "hickory resolver built without TLS support is infallible",
        )))
    }
}
