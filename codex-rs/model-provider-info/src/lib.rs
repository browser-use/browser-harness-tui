...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// The Chat Completions API exposed at `/v1/chat/completions`. This is the
    /// OpenAI-compatible format spoken by LiteLLM, OpenRouter, Ollama, vLLM, and
    /// most third-party gateways. Browser Harness re-adds this (upstream removed
    /// it in discussion #7782) so the agent can drive any model behind a
    /// LiteLLM-style proxy, not just OpenAI Responses endpoints.
    Chat,
}
